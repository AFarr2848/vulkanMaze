...
inline void carveFrom(int cx, int cy, vector<vector<cell>> &maze) {
  int nx, ny;
...
inline vector<vector<cell>> makeMaze(int mazeSize) {
...
inline vector<float> mazeToFloats(float width, float height, float wallHeight, vector<vector<cell>> maze) {
...
inline vector<glm::mat4> test(float xScale, float yScale, float zScale, vector<vector<cell>> maze) {
...
inline vector<float> mazeToBounds(float xScale, float yScale, float zScale, vector<vector<cell>> maze) {
...
inline vector<glm::mat4> mazeToTransforms(float xScale, float yScale, float zScale, vector<vector<cell>> const &maze) {
  float cx = xScale / maze.at(0).size();
  float cz = zScale / maze.size();
  glm::mat4 model;
  float wallThickness = 0.1f;
